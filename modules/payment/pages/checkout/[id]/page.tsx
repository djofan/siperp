  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-12 sm:py-20">
      <section className="mx-auto max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-900">Mode simulasi</span>
        <h1 className="mt-5 text-2xl font-bold tracking-tight text-slate-900">Konfirmasi Pembayaran</h1>
        <p className="mt-2 text-sm leading-relaxed text-slate-600">Ini simulasi donasi. Tidak ada uang yang ditagihkan atau dipindahkan. Anda tidak perlu melakukan transfer.</p>
        ...
        <dl className="mt-6 space-y-3 text-sm text-slate-600">
          <div className="flex justify-between gap-4"><dt>Metode pembayaran</dt><dd className="text-right">{transaction.paymentMethod}</dd></div>
        ...
        <CheckoutForm key={transaction.id} transaction={{ id: transaction.id, trackingCode: transaction.trackingCode, status: transaction.status, moduleSource: transaction.moduleSource }} />
      </section>
    </main>
  );
}
