...
export default async function LazsipProgramPage({
  ...
  const type = tipe && TYPE_LABEL[tipe] ? tipe : "semua";
  const allItems = await listPrograms();
  const items = type === "semua" ? allItems : allItems.filter((item) => item.type === type);

  return (
    <div className="mx-auto max-w-6xl px-4 pb-20 pt-28 sm:px-6 sm:pt-32">
      <BackLink href="/lazsip#program">Kembali ke Beranda</BackLink>
      ...
      <div className="mt-8">
        <FilterChips
          ...
          value={type}
          buildHref={(value) => (value === "semua" ? "/lazsip/program" : `/lazsip/program?tipe=${value}`)}
        />
      </div>

      <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <ProgramCard key={item.id} id={item.id} title={item.title} description={item.description} image={item.image} type={item.type} registrationOpen={item.registrationOpen} />
        ))}
      </div>

      {items.length === 0 && <p className="mt-8 text-sm text-lazsip-primary-800/60">Belum ada program untuk tipe ini.</p>}
    </div>
  );
}
