 ...
 * provisional sampai provider storage produksi diputuskan (sama status seperti upload LAZSIP).
 */
...
export async function POST(request: Request) {
  ...
  const uploadDir = path.join(process.cwd(), "public", "uploads", "sip");
  await mkdir(uploadDir, { recursive: true });

  const buffer = Buffer.from(await file.arrayBuffer());
  await writeFile(path.join(uploadDir, filename), buffer);

  return NextResponse.json({ url: `/uploads/sip/${filename}` }, { status: 201 });
}
