...
import { createBlog } from "@/modules/sip/api/blog";
...
export async function POST(request: Request) {
  ...
  const blog = await createBlog({ title, content, image, isPinned, status });
  return NextResponse.json({ ok: true, id: blog.id }, { status: 201 });
}
