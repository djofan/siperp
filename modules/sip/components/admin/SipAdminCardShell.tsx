export function SipOverlayCard({
  image,
  title,
  onClick,
  meta,
...
export function SipOverlayIconButton({
  onClick,
  label,
  disabled,
  tone = "light",
...
export function SipOverlayActions({
  onEdit,
  onDelete,
  deleting = false,
}: {
  ...
  return (
    <>
      {onEdit && (
        <SipOverlayIconButton onClick={onEdit} label="Edit" disabled={deleting}>
        ...
        </SipOverlayIconButton>
      )}
      {onDelete && (
        <SipOverlayIconButton onClick={onDelete} label="Hapus" disabled={deleting} tone="danger">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-3 w-3">
